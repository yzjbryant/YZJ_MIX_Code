...
for k in range(len(Target)):
    ...
    print(Target[k], np.mean(cross_val_score(lasso, X=X_test, y=Y_pred, cv=10)),
          np.std(cross_val_score(lasso, X=X_test, y=Y_pred, cv=10)))
    p1.append(Target[k])
    p2.append(np.mean(cross_val_score(lasso, X=X_test, y=Y_pred, cv=10)))
    p3.append(np.std(cross_val_score(lasso, X=X_test, y=Y_pred, cv=10)))
    ...
    plt.xlim(0, 0.1)
    plt.ylim(0, 0.1)
    os.chdir('F:\MOFS_Extra_Data\ML_Graph\LASSO_Graph')
    ...
    # importance=lasso.feature_importances_
