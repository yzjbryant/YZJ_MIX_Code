...
sns.heatmap(correlations,vmax=1.0,center=0,fmt='.2f',square=True,linewidths=.5,
            annot=False,cbar_kws={"shrink":.70})
plt.savefig('1.png')
...
for k in range(len(Target)):
    ...
    R2 = []
    MAPE = []
    GBR=GradientBoostingRegressor(loss='ls',learning_rate=0.1,n_estimators=100,subsample=1.0,criterion='friedman_mse',min_samples_split=2,
                                  min_samples_leaf=1,min_weight_fraction_leaf=0.,max_depth=3,min_impurity_decrease=0.,min_impurity_split=None,
    ...
    print(Y_pred)
    # print(Target[k],np.mean(cross_val_score(GBR, X=X_test, y=Y_pred, cv=10)),
    #       np.std(cross_val_score(GBR, X=X_test, y=Y_pred, cv=10)))
    p1.append(Target[k])
    p2.append(np.mean(cross_val_score(GBR, X=X_test, y=Y_pred, cv=10)))
    p3.append(np.std(cross_val_score(GBR, X=X_test, y=Y_pred, cv=10)))
    from openpyxl import load_workbook
    os.chdir('F:/MOFS_Extra_Data/ML_Graph/1_Graph')
