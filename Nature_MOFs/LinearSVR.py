...
for k in range(len(Target)):
    ...
    R2 = []
    MAPE = []
    linearsvr = LinearSVR(epsilon=0.0, tol=0.0001, C=1.0,loss='epsilon_insensitive',fit_intercept=True,
                          intercept_scaling=1.0,dual=True, verbose=0, random_state=None,max_iter=1000)
    linearsvr.fit(X_train, Y_train)
    Y_pred = linearsvr.predict(X_test)
    print(Target[k], np.mean(cross_val_score(linearsvr, X=X_test, y=Y_pred, cv=10)),
          np.std(cross_val_score(linearsvr, X=X_test, y=Y_pred, cv=10)))
    p1.append(Target[k])
    p2.append(np.mean(cross_val_score(linearsvr, X=X_test, y=Y_pred, cv=10)))
    p3.append(np.std(cross_val_score(linearsvr, X=X_test, y=Y_pred, cv=10)))
    from openpyxl import load_workbook

    os.chdir('F:/MOFS_Extra_Data/ML_Graph/LinearSVR_Graph')
