...
for k in range(len(Target)):
    ...
    R2 = []
    MAPE = []
    rfr= RandomForestRegressor(n_estimators=20, criterion='mse',max_depth=None, min_samples_split=2,min_samples_leaf=1,
                               min_weight_fraction_leaf=0.0,max_features='auto', max_leaf_nodes=None, bootstrap=True,
                               oob_score=False, n_jobs=1, random_state=None, verbose=0,warm_start=False)
    rfr.fit(X_train, Y_train)
    Y_pred = rfr.predict(X_test)

    print(Target[k], np.mean(cross_val_score(rfr, X=X_test, y=Y_pred, cv=10)),
          np.std(cross_val_score(rfr, X=X_test, y=Y_pred, cv=10)))
    p1.append(Target[k])
    p2.append(np.mean(cross_val_score(rfr, X=X_test, y=Y_pred, cv=10)))
    p3.append(np.std(cross_val_score(rfr, X=X_test, y=Y_pred, cv=10)))
    from openpyxl import load_workbook

    os.chdir('F:/MOFS_Extra_Data/ML_Graph/RF_Graph')
